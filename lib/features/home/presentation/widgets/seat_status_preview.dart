...
class SeatStatusPreview extends StatelessWidget {
  const SeatStatusPreview({
    super.key,
    required this.state,
    ...
    return Column(
      ...
      children: [
        ...
        if (status != null && state.error != null) ...[
          _SeatPreviewRefreshWarning(compact: compact),
          SizedBox(height: compact ? 8 : 10),
        ],
        if (status == null && state.error == null)
          const Expanded(child: CampusPreviewLoadingSkeleton())
        else if (state.error != null && status == null)
          Expanded(
            child: CampusPreviewMessage(
...
class _SeatPreviewRefreshWarning extends StatelessWidget {
  const _SeatPreviewRefreshWarning({required this.compact});
